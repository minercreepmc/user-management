...
let SignInMapper = class SignInMapper {
    toDomain(command) {
        const domainOptions = {};
        if (command.email) {
            domainOptions.email = new user_1.UserEmailValueObject(command.email);
        }
        if (command.password) {
            domainOptions.password = new user_1.UserPasswordValueObject(command.password);
        }
